...
/// and the exact-float-IP ground-truth builder — the scalar `dot_f32` reduction doesn't vectorize
/// (float assoc), so this is ~8x on the hot float dot. Falls back to scalar off-AVX2.
#[inline]
